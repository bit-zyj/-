...
def denoising_auto_encoder():
    ...
    weights = {
        ...
        'encoder_h3': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_3], stddev=0.1)),
        'decoder_h3': tf.Variable(tf.random_normal([n_hidden_3, n_hidden_2], stddev=0.1)),
        'decoder_h2': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_1], stddev=0.1)),
        'decoder_h1': tf.Variable(tf.random_normal([n_hidden_1, n_input], stddev=0.1)),
    }
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(num_batch):
                ...
                _, loss = sess.run([optimizer, cost], feed_dict={X: batch_x_noise, y_true: batch_x})
                total_loss += loss
            # 打印信息
        ...
        reconstruction = sess.run(y_pred, feed_dict={X: test_noisy})
